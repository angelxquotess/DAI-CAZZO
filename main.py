...
class JarvisLive:
    ...
    def _on_new_message(self, platform: str, contact_name: str):
        ...
        Inietta nel Gemini Live un turno di sistema che:
          1) annuncia vocalmente "Signore, nuovo messaggio <piattaforma> da <X>"
             (oltre alla TTS gia' emessa dal poller).
          2) ricorda al modello che se l'utente dice "rispondi con <testo>"
             deve chiamare IMMEDIATAMENTE il tool 'send_message' con
             receiver=<X>, platform=<piattaforma>, message_text=<testo>.
        """
        ...
        # Memorizza anche nello stato condiviso (lo fa gia' il poller per
        # WhatsApp/Telegram/Discord/Instagram, ma duplicare e' idempotente).
        try:
            from actions.message_state import set_last_incoming
            set_last_incoming(plat, cn)
        except Exception:
            pass
        ctx = (
            f"[NUOVO MESSAGGIO {plat_label.upper()}]\n"
            f"E' arrivato un nuovo messaggio su {plat_label} da: {cn}\n"
            f"ISTRUZIONI:\n"
            f"1) Annuncia VOCALMENTE in italiano, una sola frase breve: "
            f"\"Signore, nuovo messaggio {plat_label} da {cn}\".\n"
            f"2) Da questo momento e finche' l'utente non cambia argomento, "
            f"se l'utente dice \"rispondi con <testo>\", \"rispondigli "
            f"<testo>\", \"digli <testo>\" o frasi equivalenti, devi "
            f"IMMEDIATAMENTE chiamare il tool send_message con "
            f"receiver=\"{cn}\", platform=\"{plat}\" e message_text=<testo "
            f"detto dall'utente>. Non chiedere conferma, invia subito.\n"
            f"3) Dopo l'invio, conferma brevemente: \"Risposta inviata a "
            f"{cn} su {plat_label}, signore.\""
        )
        try:
